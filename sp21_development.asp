...
if proses <> "" then	
		...
		elseif proses="Hantar" then 
			...
			if request.cookies("gnop")="9678" then
				tambah9678
			else
				tambah
			end if
...
if not cc1.bof and not cc1.eof then
	if isnull(cc1("tkh_sah")) then
		...
		tkh_tentu_kadar = request.form("tkh")
		...
		if cdbl(masa_mulaKira)< 6 then
			tkh_tmp = DateAdd("d",-1,Cdate(request.form("tkh")))
			tkh_tmp_year = DatePart("yyyy",tkh_tmp)
			tkh_tmp_month = DatePart("m",tkh_tmp)
			if len(tkh_tmp_month)="1" then tkh_tmp_month="0"& tkh_tmp_month
			tkh_tmp_day = DatePart("d",tkh_tmp)
			if len(tkh_tmp_day)="1" then tkh_tmp_day="0"& tkh_tmp_day
			tkh_tentu_kadar = tkh_tmp_month &"/"& tkh_tmp_day &"/"& tkh_tmp_year
		end if
		...
		'2. check weekend
		r="select tarikh from kelepasan_am where to_char(tarikh,'mm/dd/yyyy') like '"& tkh_tentu_kadar &"'"
		set rs1=objConn.Execute(r)
		if not rs1.bof and not rs1.eof then
			statusHari="CU"
		else
			'q="select tarikh from sabtu where to_char(tarikh,'mm/dd/yyyy') like '"& tkh &"'"
			q="select decode(to_char(to_date('"& tkh_tentu_kadar &"','mm/dd/yyyy'),'DY'),'SUN','SA','SAT','SA','P') as statusHari from dual "

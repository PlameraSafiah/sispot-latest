...
blnthn=TodayMonth&TodayYearNumber
  ...
  ' end if
   ...
   tahun = oq2b ("tahun")
    ...
    'semak jabatan kelulusan - nadia (19042016)
	'if pekd=9678 or pekd=11208 then
...
<TABLE border=1 borderColor=black cellPadding=1 cellSpacing=0 rules=all class="hd">
  ...
  <option value="">Pilih Tahun</option>
		  ...
		  if not isnull(tahun) then tahuncheck = Cstr(tahun)
		  if not isnull(oq2b("tahun")) then tahuncheck2 = Cstr(oq2b("tahun"))
		  %>
          <option <%if tahuncheck = tahuncheck2 then%> selected <%end if%>value="<%=oq2b("tahun")%>"><%=oq2b("tahun")%></option>
          <%
            oq2b.MoveNext
            loop
           %>
...
<br>
  ...
  <TABLE border=1 borderColor=black cellPadding=1 cellSpacing=0 rules=all class="hd">
    <tr align="center">
    <td colspan="2">
   <form name="test<%=bil%>"  method="post" action="sp16a.asp?bulan=<%=bulan%>&tahun=<%=tahun%>&kod=<%=kod%>&amenu='sp16a'" target="_new">

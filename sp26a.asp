...
<%Set objConn = Server.CreateObject("ADODB.Connection")
 ...
 ss = " select id_ak ,pemohon, penyelia , unit , ptj , keterangan , nota , tahun , bulan"
 ss = ss & " from payroll.arahan_kerja where id_ak='"&id&"' " 
  ...
  if not rspp.eof then
	...
	lulus = "select no_pekerja from payroll.unit_sispot_pelulus where lokasi = '"& ptj &"' and no_pekerja <> '"&pemohon&"' "
	set objlulus = objconn.execute(lulus)
	
	if not objlulus.eof then
	
	pl = objlulus ("no_pekerja")
	
	
		pp = " select no_pekerja , nama , jawatan from payroll.paymas " 
		pp = pp & " where no_pekerja = '"&pl&"' " 
		Set rs = objConn.Execute (pp)
		if not rs.eof then
		nama1 = rs("nama")
		jaw1 = rs("jawatan")

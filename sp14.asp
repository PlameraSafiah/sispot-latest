...
<body>
<%response.cookies("amenu") = "sp14.asp"%>


<FORM name="kew" action="sp14.asp" method="post" >
  ...
  if proses = "Simpan" then
     bilrec = request.form("bilrec")
        ' ss = "delete majlis.kebenaran_2002 where no_pekerja = '"& pekerja &"' and sistem = 'sp'  "
		 ss = "delete majlis.kebenaran_2002 where no_pekerja = '"& pekerja &"' and sistem = 'sp' and skrin not like 'sp14%' "
...
<%   
	...
	' response.write b2
	 ...
	 mm2 = "select jabatan,initcap(nama) nama from payroll.tetap_sambilan where no_pekerja = '"& pekerja &"' "
     Set mmm2 = objConn.Execute(mm2) 
	 jabatan2 = mmm2("jabatan")
	 
	 
	 if jabatan2 = "101" or jabatan2 = "107" then 
     b3 =      "select rowid,kod,keterangan,paras,tamat "
     b3 = b3 & "  from majlis.menu_2002 where kod like '"& kod2 &"'||'%' and paras=3 and sistem = 'sp' and kod not like 'sp14%' and kod not like 'sp15%' order by kod "
	 end if

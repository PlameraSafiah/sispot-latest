...
<%'response.cookies("amenu") = "sp23_t.asp"%>
<html>
<head>
...
cc0v="select 'x' no_pekerja from payroll.proses_ot where no_pekerja="& pekerja &" and to_char(tkh_ot,'mmyyyy')='"& mid(tkh_ot,1,2) & mid(tkh_ot,5,4) &"' and sah_individu='T'"
...
sub papar ()
q1="select id_ak from payroll.arahan_kerja where ptj="& ptj &" and tkh_sah is null and tkh_lulus is null and pemohon="& carianpemohon  &" order by tahun desc,bulan desc,id_ak desc" 'and id_ak in "
'q1 = q1 & "(select id_ak from jadual_ot) order by tahun desc,bulan desc,id_ak"
set oq1 = objConn.Execute(q1)
%>
<br>
<form name="test" method="post" action="sp23_t.asp">
...
<TABLE border=1 borderColor=black cellPadding=1 cellSpacing=0 rules=all class="hd">
  ...
  <form name="test0" method="post" action="sp23_t.asp">
...
%>
  <form name="test<%=bil%>"  method="post" action="sp23_t.asp">

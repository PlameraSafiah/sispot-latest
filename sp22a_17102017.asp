...
if not cc1.bof and not cc1.eof then
	if isnull(cc1("tkh_sah")) then	
		...
		nota = replace(request.form("nota"),"'","''")	
		 ...
		 i1="update payroll.arahan_kerja set penyelia='"& fpenyelia &"',unit='"& funit &"',ptj='"& ptj &"',kategori='"& fkategori &"',"
		 i1= i1 & "keterangan='"& fketerangan &"',nota='"& nota &"',tkh_input=to_date('"& todaydate &"','ddmmyyyy HH24:MI'), "
		 i1= i1 & "daerah='"& fdaerah &"',parlimen='"& fparlimen &"',dun='"& fdun &"',tempat='"& tempat &"' "
...
<TR bgcolor="<%=color1%>" > 
  ...
  <td bgcolor="F4FAFA"> 
     ...
     zv4 = " select upper(keterangan) ket from payroll.keterangan_kerja where kategori = '"& fkategori1 &"' order by ket  "

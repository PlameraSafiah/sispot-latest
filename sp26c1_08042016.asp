...
<%Set objConn = Server.CreateObject("ADODB.Connection")
  ...
  p = p & "  from payroll.arahan_kerja where pemohon='"&pemohon&"' and pengesahan='Y' order by id_ak"
...
'  tamat = rsqqq ("masa_tamat")
  ...
  qq = " select to_char(tkh_ot,'dd/mm/yyyy') as tkh_ot , to_char(masa_mula_angg,'HH24:MI') as masa_mula , to_char(masa_tamat_angg,'HH24:MI') as masa_tamat "
  qq = qq & " from payroll.pekerja_ot " 
  qq = qq & " where id_ak = '"&id&"'  "
  qq =qq & " order by tkh_ot  " 
  Set rsqq = objConn.Execute (qq)
   ...
   bil = bil + 1
   tkh_ot = rsqq ("tkh_ot")
  mula1 = rsqq ("masa_mula")
  tamat1 = rsqq ("masa_tamat")
	
	
  qqq = " select distinct to_char(masa_mula_angg,'HH24:MI') as masa_mula , to_char(masa_tamat_angg,'HH24:MI') as masa_tamat "
  qqq = qqq & " from payroll.pekerja_ot " 
  qqq = qqq & " where id_ak = '"&id&"' and to_char(tkh_ot,'dd/mm/yyyy') = '"&tkh_ot&"' and to_char(masa_mula_angg,'HH24:MI') = '"&mula1&"'  "
  qqq = qqq & " and  to_char(masa_tamat_angg,'HH24:MI') = '"&tamat1&"' "
  Set rsqqq = objConn.Execute (qqq)
   'response.write qqq
  mula = rsqqq ("masa_mula")
  tamat = rsqqq ("masa_tamat")
...
<table align="center" border="0" cellpadding="0" cellspacing="0" width="70%" class="page-break">
<tr>
  <td align="center" valign="top" colspan="3">
  </td>
</tr>

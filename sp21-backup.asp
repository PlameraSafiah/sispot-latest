...
q1="select max(substr(id_ak,10,3)) siri from payroll.arahan_kerja "
q1 = q1 & " where id_ak like  '"&ptj&"'||'"&thn&"'||'"&bln&"'||'%' "
set oq1 = objConn.Execute(q1)

	if oq1.bof and oq1.eof then	 
        d1 = " select '"& ptj &"'||'"& thn &"'||'"& bln &"'||'001' dok from dual "
        ...
        d1 = " select '"& ptj &"'||'"& thn &"'||'"& bln &"'||lpad('"&zsiri&"',3,'0') dok from dual "
        Set od1 = objConn.Execute(d1)	
...
<TABLE border=1 borderColor=black cellPadding=1 cellSpacing=0 rules=all class="hd">
  <TBODY>
  <form name="test0" method="post" action="sp21.asp">
    <tr> 

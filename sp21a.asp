...
q0 = q0 & "ptj="& ptj &" and pemohon="& no_pekerja &" and unit like '"& funit1 &"' and upper(keterangan_1) like upper('"& fketerangan1 &"') and "
...
'-----
	 ...
	 If fkategori<>"" and fketerangan<>"" and fdaerah<>"" and fpenyelia <> "" then 'mimi tambah penyelia jadi wajib - 04092018
	 
	 i1="insert into payroll.arahan_kerja (id_ak,penyelia,pemohon,pengesahan,tkh_sah,unit,ptj,kategori,keterangan_1,nota,tkh_input,tahun,bulan,daerah,parlimen,dun,tempat) values "
...
<%	 else %>
<script language="JavaScript">
	alert("Sila lengkapkan ruangan Penyelia, Kategori ,Keterangan Kerja dan Daerah.");
	document.masa0.tkh.focus();
</script>
<%
end if 
...
wajibField="&nbsp;<font style='font-family: Trebuchet MS; font-size: 13pt; color:red;'>*&nbsp;</font>"
%>
<br>
<TABLE border=1 borderColor=black cellPadding=1 cellSpacing=0 rules=all class="hd">
 ...
 <tr><td align="right" colspan="2" ><font style="font-family: Trebuchet MS; font-size: 10pt; color:red;"><a href="manual_sp21a.pdf" target="_new"> Manual Pengguna untuk menu ini </a></font></td></tr>   
 <tr> 
    ...
    <tr>
      <td class="hd">Jabatan / Unit <%=wajibField%></td>
...
'-----
 ...
 Sekiranya tiada dalam pilihan,sila daftar unit anda
    ...
    <tr> 
      <td class="hd">Penyelia <%=wajibField%></td>
      <td> 
...
'-----
	...
	%>
     <TR bgcolor="<%=color1%>" > 
  <TD  align="left" bgcolor="<%=color1%>" width="40%"><b>Kategori <%=wajibField%></b></TD>
  <td valign="center" bgcolor="F4FAFA">
  <select name="fkategori" id="fkategori" onChange="submitForm6(this)">
           <option value="">Pilih Kategori -- </option>
           
           <% 'saf 14022018 tambah filter by ptj
		   zv2 = "select distinct(kategori) kod from payroll.keterangan_kerja where ptj='"&ptj&"' "
		   zv2 = zv2 & " union"
		   zv2 = zv2 & " select distinct(kategori) kod from payroll.kategori_kerja  where ptj='"&ptj&"' order by kod"

'zv2 = "select distinct(kategori) kod from payroll.kategori_kerja order by kategori"
   		   Set rszv2 = objConn.Execute(zv2) 

		   do while not rszv2.eof 
		   
		   kodkategori = rszv2("kod")
...
'-----
		   %>
           </select> <input type="hidden" name="fkategori1" value="<%=session("fkategori2")%>" id="fkategori1">
...
<TR bgcolor="<%=color1%>" > 
  <TD  align="left" bgcolor="<%=color1%>" width="40%"><b>Keterangan <%=wajibField%></b></TD>
  <td bgcolor="F4FAFA"> 
...
'saf 14022018 tambah filter by ptj
     zv = " select upper(keterangan) ket from payroll.keterangan_kerja where kategori = '"& ffkategori1 &"' and ptj='"&ptj&"' order by ket  "
...
'-----
		   %>
           </select><input type="hidden" name="fketerangan1" value="<%=session("fketerangan2")%>" id="fketerangan2">
           
          Sekiranya tiada dalam pilihan,sila daftar 
            <a href="sp110.asp" target="_new"> di sini.</a></td>
</tr>

    
    <tr>
      <td class="hd" >Daerah <%=wajibField%></td><td>
